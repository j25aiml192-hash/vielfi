    ...
    <div className="card-hover">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-display font-bold text-white">{circle.name}</h3>
            <TierBadge tier={circle.tier} size="sm" />
          </div>
          <p className="text-grey text-sm">{circle.description}</p>
        </div>
        {circle.joined && (
      ...
      <div className="grid grid-cols-3 gap-4 mt-5 pt-4 border-t border-border">
        <div className="text-center">
          <div className="font-bold text-white">{circle.members}/{circle.maxMembers}</div>
          <div className="text-xs text-grey mt-0.5">Members</div>
        </div>
        <div className="text-center">
          <div className="font-bold text-gold">{formatINR(circle.totalPool)}</div>
          <div className="text-xs text-grey mt-0.5">Pool Size</div>
        </div>
        <div className="text-center">
          <div className="font-bold text-white">{circle.purpose}</div>
          <div className="text-xs text-grey mt-0.5">Purpose</div>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {circle.contributions.map((c) => (
              <div key={c.name} className="flex justify-between text-sm">
                <span className="text-grey">{c.name}</span>
                <span className="text-white font-medium">{formatINR(c.amount)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex gap-2 mt-4">
        ...
        {circle.joined && (
          <button className="btn-teal text-xs px-4 py-2 flex-1 justify-center">
            Manage
          </button>
        )}
      </div>
...
function CreateCircleForm({ onCreated }) {
  ...
  if (success) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="font-display font-bold text-white text-xl mb-2">Circle Created!</h3>
        <p className="text-grey text-sm">Your credit circle is now live on VeilFi.</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="text-sm text-grey mb-2 block">Circle Name *</label>
        <input required className="input" placeholder="e.g. Mumbai Artisans Circle" value={form.name} onChange={set('name')} />
      </div>
      <div>
        <label className="text-sm text-grey mb-2 block">Description *</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-sm text-grey mb-2 block">Purpose</label>
        ...
        <div>
          <label className="text-sm text-grey mb-2 block">Max Members</label>
          <input type="number" className="input" min={3} max={50} value={form.maxMembers} onChange={set('maxMembers')} />
        </div>
      </div>
      <div>
        <label className="text-sm text-grey mb-2 block">Target Pool (INR)</label>
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <h1 className="font-display font-black text-4xl text-white">
                Credit <span className="text-gradient-indigo">Circles</span>
              </h1>
          ...
          <div className="animate-fade-in">
            {joined.length === 0 ? (
              <div className="text-center py-20">
                ...
                <h3 className="font-display font-bold text-white text-xl mb-2">No circles yet</h3>
                <p className="text-grey mb-5">Browse and join a credit circle to get started</p>
        ...
        {tab === 'create' && (
          <div className="max-w-lg mx-auto card animate-slide-up">
            <h2 className="font-display font-bold text-white text-xl mb-6">Create a Credit Circle</h2>
            {!isConnected && (
