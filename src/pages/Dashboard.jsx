...
function BorrowerDashboard({ loans, loading }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Credit Score', value: creditScore, sub: tier, color: 'text-gold' },
          { label: 'Total Borrowed', value: formatINR(totalBorrowed), sub: `${loans.length} loan${loans.length === 1 ? '' : 's'}`, color: 'text-white' },
          { label: 'Total Funded', value: formatINR(totalRepaid), sub: `${totalPct}% funded`, color: 'text-teal' },
          { label: 'On-Time Payments', value: `${onTimePayments}`, sub: 'Demo repayment streak', color: 'text-indigo' },
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          {loans.map((loan) => {
            const fundedPct = loan.amount > 0 ? Math.round((loan.fundedAmount / loan.amount) * 100) : 0
            return (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1 text-sm text-grey">
                      ...
                      <span>|</span>
                      <span>{formatINR(loan.fundedAmount)} funded</span>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-sm text-grey">Next EMI</div>
                    <div className="font-display font-bold text-gold">{formatINR(loan.emiAmount)}</div>
                    <div className="text-xs text-grey">{loan.durationMonths} months at {loan.interestRate}% APR</div>
                  </div>
                </div>
                <div className="mt-4">
      ...
      <div className="card">
        <h2 className="font-display font-bold text-white mb-4">Overall Funding</h2>
        <ProgressBar value={totalPct} variant="gold" size="lg" showPct />
        <div className="flex justify-between text-xs text-grey mt-2">
          <span>{formatINR(totalRepaid)} funded</span>
          <span>{formatINR(Math.max(totalBorrowed - totalRepaid, 0))} remaining</span>
        </div>
      </div>
    </div>
  )
...
function LenderDashboard({ fundedLoans, loading }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Deployed', value: formatINR(totalDeployed), color: 'text-gold' },
          { label: 'Funded Loans', value: fundedLoans.length, color: 'text-white' },
          { label: 'Projected Returns', value: formatINR(totalReturns), color: 'text-teal' },
          { label: 'Avg APR', value: `${avgReturn}%`, color: 'text-indigo' },
      ...
      <div className="card border border-teal/20 bg-gradient-to-r from-teal/5 to-transparent flex items-center justify-between gap-4">
        <div>
          <h3 className="font-display font-bold text-white">Deploy more capital</h3>
          <p className="text-grey text-sm mt-0.5">Browse verified borrowers and earn transparent marketplace APR.</p>
        </div>
        <button
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-grey text-sm mt-1">
              You've helped <span className="text-teal font-semibold">{fundedLoans.length} borrower{fundedLoans.length === 1 ? '' : 's'}</span> access fair credit.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-display font-bold text-white">Funded Loans</h2>
            ...
            <tbody className="divide-y divide-border/50">
              {fundedLoans.map((loan) => (
                <tr key={loan.id} className="hover:bg-white/2 transition-colors">
                  <td className="py-3 font-medium text-white">{loan.borrowerName}</td>
                  <td className="py-3"><TierBadge tier={loan.tier} size="sm" /></td>
                  <td className="py-3 text-right text-grey">{formatINR(loan.fundedAmount)}</td>
                  <td className="py-3 text-right text-teal font-semibold">{loan.interestRate}%</td>
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            {showDemoNotice && (
              <div className="text-sm text-grey border border-border rounded-xl px-4 py-3 bg-card">
                {!isConnected ? 'Showing demo data - connect wallet' : 'Showing Rahul demo data'}
              </div>
            )}
            {noRole && (
              <button
        ...
        {noRole && (
          <div className="card border border-gold/30 bg-gold/5 flex items-center gap-4 mb-8">
            <div className="w-12 h-12 rounded-xl bg-gold/15 border border-gold/30 flex items-center justify-center text-gold font-display font-bold">
              VF
            </div>
        ...
        <TabSwitcher
          ...
          isBorrower={noRole || isBorrower}
          isLender={noRole || isLender}
        />
