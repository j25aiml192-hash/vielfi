      ...
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:14, overflow:'hidden' }}>
        ...
        {isConnected ? (
          <div style={{ borderTop:`1px solid ${C.border}`, padding:'10px 10px' }}>
            {[
              { icon:'≡ƒÆ╝', label:'My Portfolio',    path:'/dashboard' },
              { icon:'≡ƒôê', label:'My Investments',  path:'/dashboard' },
              { icon:'≡ƒöÉ', label:'Verify Identity', path:'/verify'   },
              { icon:'≡ƒæÑ', label:'My Circles',      path:'/circles'  },
            ].map(({ icon, label, path }) => (
          ...
          <div style={{ borderTop:`1px solid ${C.border}`, padding:'14px 18px' }}>
            ...
            >
              ...
              {connecting ? 'ConnectingΓÇª' : 'Connect Wallet'}
            </button>
          </div>
        )}
      </div>
